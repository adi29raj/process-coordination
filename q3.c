...
void course_intilization(int std_count,int crs_count)
{
    ...
    for(int i=0;i<crs_count;i++)
    {
        ...
        c[i].knowledge=(i%4);
        c[i].stuEnrolled=0;
        c[i].std_branch[0]=12;    //m comm
        c[i].std_branch[1]=12;        //phd
        c[i].std_branch[2]=24;      //b comm  
        c[i].std_branch[3]=12;    //arts
...
void *student_thread(void *arg1){
	...
	for(i = 0 ;i < 8 ; i++){
		...
		int std_branch_course = c[s_course_id].std_branch[s_branch];
        //  printf("temp2 %d %d %d\n",s_course_id,s_branch,std_branch_course);
        //  printf("%d %d\n",s[studentid].course_assigned,std_branch_course);
		// int std_branch_course = c[s_course_id].std_branch[s_branch];
		// printf(" %d %d %d \n",s_course_id,s_branch,c[s_course_id].std_branch[s_branch]);
		if(std_branch_course != 0){
			if(s[studentid].course_allocated > 0){
				...
				c[s_course_id].std_branch[s_branch]--;
...
int main(int argc,char *argv[])
{
	...
	for(i = 0 ; i < tcourses ; i++){
        fprintf(file,"%d \t \t \t\t\t %d\n",c[i].course_id+1,c[i].stuEnrolled);
		}
